...
class Data:
    ...
    def create_connection(self):
        ...
        if not db.open():
            QtWidgets.QMessageBox.critical(
                None, "Cannot open database", "Click Cancel to exit.", QtWidgets.QMessageBox.Cancel)
            return False
    ...
    def update_transaction_query(self, date, category, description, balance, status, id):
        ...
        print(f"id={id}")
        print(f"update:{id},{date},{category},{
    ...
    def get_total(self, column, filter=None, value=None):
        ...
        if query.next():
            print(f"res={column},sum={query.value(0)},value={
                  value},filter={filter}")
    ...
    def total_income(self):
        res = self.get_total(column="Balance", filter="Status", value="Income")
        print(f"res income={res}")
        return res
